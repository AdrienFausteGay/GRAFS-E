...
class NitrogenFlowModel:
    ...
    def compute_fluxes(self):
        ...
        WASTE_PWR_MWh_per_ktN = (
            float(df_global.loc["Green waste methanization power (MWh/ktN)", "value"])
            if "Green waste methanization power (MWh/ktN)" in df_global.index
            else 0.0
        )
        ...
        delta_energy_terms = []  # (fac, sum(delta_fac), n_pairs_fac)
        ...
        conv_prod_MWh_per_ktN = {
            ...
            * 1000.0
            / max(1e-9, float(df_prod.loc[p, "Nitrogen Content (%)"]) / 100.0)
            for p in df_prod.index
        }
        ...
        for facility, row in df_energy.iterrows():
            ...
            # Tokens qui comptent comme "déchets", uniquement s'ils sont explicitement dans la diète
            WASTE_TOKENS = {"waste", "green waste", "municipal waste", "organic waste"}

            def _is_waste_token(x):
                return str(x).strip().lower() in WASTE_TOKENS
            ...
            conv_prod_MWh_per_ktN = {
                p: float(df_prod.loc[p, "Energy Production (MWh/tFW)"])
                * 1000.0
                / (float(df_prod.loc[p, "Nitrogen Content (%)"]) / 100.0)
                for p in df_prod.index
            }
            ...
            E_MWh_products = (
                lpSum(
                    x_fac_prod[p]
                    * (
                        float(df_prod.loc[p, "Energy Production (MWh/tFW)"])
                        * 1000.0
                        / (float(df_prod.loc[p, "Nitrogen Content (%)"]) / 100.0)
                    )
            ...
            E_MWh_excreta = (
                lpSum(
                    x_fac_excr[e]
                    * (
                        float(df_excr.loc[e, "Energy Production (MWh/tFW)"])
                        * 1000.0
                        / (float(df_excr.loc[e, "Nitrogen Content (%)"]) / 100.0)
                    )
            ...
            E_MWh_waste = (
                (N_waste_fac * WASTE_PWR_MWh_per_ktN)
            ...
            E_MWh_products_import = (
                lpSum(
                    I_energy_vars[(p, facility)] * conv_prod_MWh_per_ktN[p]
                    for (p, f) in I_energy_vars
                    if f == facility
                )
                if any(f == facility for (_, f) in I_energy_vars)
                else 0
            )
            ...
            # Diète de l'infrastructure (pairs_fac en utilisant le NOM de l'infra)
            pairs_fac = []
            for _, r in fac_diet_df.iterrows():
                prop = float(r["Proportion"])
                prop = prop / 100.0 if prop > 1.0 else prop
                pairs_fac.append((facility, prop, tuple(r["Products"])))

            delta_fac = LpVariable.dicts(
                f"delta_{facility}",
                [(facility, tuple(pL)) for _, _, pL in pairs_fac],
                lowBound=0,
                cat=LpContinuous,
            )

            # N total vers l’infra (local + excréta + waste + imports)
            N_to_fac_total = (
                (lpSum(x_fac_prod.values()) if len(x_fac_prod) else 0)
                + (lpSum(x_fac_excr.values()) if len(x_fac_excr) else 0)
                + ((N_waste_fac) if (N_waste_fac is not None) else 0)
                + (
                    lpSum(
            ...
            for _, prop, prod_list in pairs_fac:
                N_group = 0
                for it in prod_list:
                    if it in x_fac_prod:
                    ...
                    elif _is_waste_token(it) and (N_waste_fac is not None):
                        N_group += N_waste_fac
                    if (it, facility) in I_energy_vars:
                        N_group += I_energy_vars[(it, facility)]
                dv = delta_fac[(facility, tuple(prod_list))]
                prob += N_group - prop * N_to_fac_total <= dv
                prob += prop * N_to_fac_total - N_group <= dv

            delta_energy_terms.append(
                (facility, lpSum(delta_fac.values()), max(1, len(pairs_fac)))
            )
            ...
            # -- Pénalités intra-groupe (tous items, y compris excréta & waste)
            penalite_culture_fac = LpVariable.dicts(
                f"penalite_culture_{facility}",
                [(facility, prop, it) for _, prop, L in pairs_fac for it in L],
                lowBound=0,
                cat=LpContinuous,
            )

            for _, prop, L in pairs_fac:
                # On ne compte que les items VRAIMENT autorisés pour cette infrastructure
                allowed_items = []
                for it in L:
                    if (
                        (it in x_fac_prod)
                        or (it in x_fac_excr)
                        or (_is_waste_token(it) and (N_waste_fac is not None))
                ...
                k = max(
                    1, len(allowed_items)
                )  # nombre d'items réellement disponibles dans ce groupe
                cible_item = (prop * N_to_fac_total) / k  # cible linéaire

                for it in allowed_items:
                    # allocation réelle de l'item vers l'infra (local/excréta/waste/imports)
                    if it in x_fac_prod:
                        ...
                        # Par construction 'allowed_items' assure qu'on n'arrive pas ici,
                        # mais on laisse ce garde-fou
                        continue

                    pv = penalite_culture_fac[(facility, prop, it)]
                    # |alloc_it - cible_item| <= pv  (linéarisé)
                    prob += (
                        alloc_it - cible_item <= pv,
                        f"Penalite_Energy_Plus_{facility}_{hash((prop, it))}",
                    )
                    prob += (
                        cible_item - alloc_it <= pv,
                        f"Penalite_Energy_Moins_{facility}_{hash((prop, it))}",
                    )

            n_items_fac = max(1, sum(len(L) for _, _, L in pairs_fac))
        ...
        #    (remplace ta ligne x_meth_term)
        for p in df_prod.index:
            consumers_with_p = [c for (prod, c) in valid_pairs if prod == p]
            x_energy_take_p = (
                lpSum(energy_prod_vars_by_p[p]) if energy_prod_vars_by_p[p] else 0
            )
            prodN = float(df_prod.loc[p, "Available Nitrogen Production (ktN)"])
            prob += (
        ...
        energy_inputs_total = (
            lpSum(
                (sum_fac_deltas / n_fac_pairs)
                for (_, sum_fac_deltas, n_fac_pairs) in delta_energy_terms
            )
            if delta_energy_terms
            else 0
        )

        # Dénominateur de normalisation : tu gardes le tien
        imp_norm = max(
            1.0,
            df_cons["Ingestion (ktN)"].sum()
            - df_prod.loc[
                df_prod["Sub Type"] != "non edible meat", "Nitrogen Production (ktN)"
            ].sum(),
        )

        raw_import_term = (
            (lpSum(I_vars.values()) if len(I_vars) else 0)
            + (lpSum(I_energy_vars.values()) if len(I_energy_vars) else 0)
        ) / imp_norm
        ...
        if use_milp_no_swap:
            for p in df_prod.index:
                ...
                I_p_energy = lpSum(
                    I_energy_vars[(pp, fac)] for (pp, fac) in I_energy_vars if pp == p
                )
                I_p_total = I_p_cons + I_p_energy
                prob += I_p_total <= M[p] * y_vars[p], f"NoSwap_I_{p}"
        ...
        if self.debug:
            ...
            def _sum_prefix(prefix):  # somme var.value par préfixe
                s = 0.0
                for v in prob.variables():
                    if v.name.startswith(prefix) and v.varValue:
                        s += float(v.varValue)
                return s

            # ---------- 1) Familles "non-énergie" ----------
            # Diet deviation
            raw_dev = (
                float(value(lpSum(delta_vars.values()))) if len(delta_vars) else 0.0
            )
            norm_dev = raw_dev / n_pairs
            w_dev = float(poids_penalite_deviation) * norm_dev
            ...
            w_cult = float(poids_penalite_culture) * norm_cult

            # Fair local split total (consommateurs + énergie si présent)
            fair_term_val = float(value(fair_term)) if "fair_term" in locals() else 0.0
            fair_energyval = (
                float(value(fair_term_energy))
                if "fair_term_energy" in locals()
                else 0.0
            )
            fair_total = fair_term_val + fair_energyval
            norm_fair = fair_total / n_prod
            w_fair_total = float(w_fair) * norm_fair

            # Import brut
            raw_import = float(
                value((lpSum(I_vars.values()) if len(I_vars) else 0))
                + value((lpSum(I_energy_vars.values()) if len(I_energy_vars) else 0))
            )
            norm_import = raw_import / float(imp_norm)
            w_import = float(poids_import_brut) * norm_import

            # ---------- 2) Familles "énergie" (df_energy) ----------
            # a) Déviation d'énergie (somme des {facility}_energy_dev, déjà normalisées |E-T|/T)
            energy_dev_norm_sum = 0.0
            for fac in df_energy.index:
                energy_dev_norm_sum += _sum_prefix(f"{fac}_energy_dev")
            w_energy_prod = W_ENERGY_PROD * energy_dev_norm_sum

            # b) Diètes d'inputs des infrastructures : W_ENERGY_INPUT * (sum delta_fac / n_pairs_fac) agrégé
            w_energy_inputs = 0.0
            for fac, row in df_energy.iterrows():
                diet_name = str(row["Diet"])
                n_pairs_fac = max(1, len(diets[diets["Consumer"] == diet_name]))
                raw_delta_fac = _sum_prefix(f"delta_{fac}_")
                w_energy_inputs += W_ENERGY_INPUT * (raw_delta_fac / n_pairs_fac)

            # c) Pénalités intra-groupe des infrastructures : somme (poids_penalite_culture / n_items_fac * sum penalite_fac)
            w_energy_penalite = 0.0
            for fac, row in df_energy.iterrows():
                diet_name = str(row["Diet"])
                n_items_fac = 0
                for _, r in diets[diets["Consumer"] == diet_name].iterrows():
                    n_items_fac += len(r["Products"])
                n_items_fac = max(1, n_items_fac)
                raw_pen_fac = _sum_prefix(f"penalite_culture_{fac}_")
                w_energy_penalite += float(poids_penalite_culture) * (
                    raw_pen_fac / n_items_fac
                )

            # ---------- 3) Objectif total (exact) ----------
            ...
            families = {
                "diet_deviation": {
                    ...
                    "weighted": w_dev,
                    "share_%": _share(w_dev, obj_val),
                    "normalizer": {"n_pairs": n_pairs},
                },
                "intra_group_distribution": {
                    "raw": raw_cult,
                    "norm": norm_cult,
                    "weight": float(poids_penalite_culture),
                    "weighted": w_cult,
                    "share_%": _share(w_cult, obj_val),
                    "normalizer": {"n_products": n_prod},
                },
                "fair_total": {
                    "raw_post_product_scale": fair_total,
                    "norm": norm_fair,
                    "weight": float(w_fair),
                    "weighted": w_fair_total,
                    "share_%": _share(w_fair_total, obj_val),
                    "normalizer": {"n_products": n_prod},
                },
                "import_brut": {
                    "raw": raw_import,
                    ...
                    "weighted": w_import,
                    "share_%": _share(w_import, obj_val),
                    "normalizer": {"denom_import": imp_norm},
                },
                "energy_production_all": {
                    "norm_sum": energy_dev_norm_sum,
                    "weight": W_ENERGY_PROD,
                    "weighted": w_energy_prod,
                    "share_%": _share(w_energy_prod, obj_val),
                },
                "energy_inputs_all": {
                    "weighted": w_energy_inputs,
                    "share_%": _share(w_energy_inputs, obj_val),
                    "note": "Already aggregated with W_ENERGY_INPUT / n_pairs_fac per facility.",
                },
                "energy_intragroup_all": {
                    "weighted": w_energy_penalite,
                    "share_%": _share(w_energy_penalite, obj_val),
                    "note": "Sum over facilities of (poids_penalite_culture / n_items_fac) * sum(penalite_culture_fac).",
                },
            }
        ...
        # imports (produits) -> infrastructure (bioraffineries)
        for (p, fac2), var in I_energy_vars.items():
            v = float(var.varValue or 0.0)
            if v <= 1e-6:
                continue
            fac_type2 = str(df_energy.loc[fac2, "Type"])
            allocations.append(
                {
                    "Product": p,
                    "Consumer": fac2,
        ...
        for fac in df_energy.index:
            ...
            for _, row in fac_diet_df.iterrows():
                ...
                for it in prods:
                    ...
                    elif _is_waste_token(it) and (
                        energy_vars[fac]["waste"] is not None
                    ):
                        N_group += float(value(energy_vars[fac]["waste"]))
                    if (it, fac) in I_energy_vars:
                        N_group += float(I_energy_vars[(it, fac)].varValue or 0.0)

                alloc_pct = 100.0 * N_group / N_total if N_total > 1e-12 else 0.0
                exp_pct = 100.0 * prop
        ...
        for fac in df_energy.index:
            ...
            for p, var in energy_vars[fac]["prod"].items():
                ...
                mwh_per_ktn = (
                    float(df_prod.loc[p, "Energy Production (MWh/tFW)"])
                    * 1000.0
                    / max(eps, float(df_prod.loc[p, "Nitrogen Content (%)"]) / 100.0)
                )
            ...
            for (p, f), var in I_energy_vars.items():
                ...
                if N <= 1e-6:
                    continue
                E_gwh = N * (conv_prod_MWh_per_ktN[p] / 1000.0)
            ...
            for e, var in energy_vars[fac]["excr"].items():
                ...
                mwh_per_ktn = (
                    float(df_excr.loc[e, "Energy Production (MWh/tFW)"])
                    * 1000.0
                    / max(eps, float(df_excr.loc[e, "Nitrogen Content (%)"]) / 100.0)
                )
                E_gwh = N * (mwh_per_ktn / 1000.0)
                rows_energy.append(
                    {
        ...
        if not self.df_energy_flows.empty:
            for fac in df_energy.index:
                ...
                if fac_type.lower() == "methanizer":
                    ...
                    flux_generator.generate_flux(source, {"other sectors": 1})
