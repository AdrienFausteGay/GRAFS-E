...
annees_disponibles = ['1852', '1885', '1906', 
                      '1929', '1946', '1955',
                      '1965', '1970', '1974',
                      '1978', '1981', '1985',
                      '1989', '1993', '1997',
...
ext = ['NH3 volatilization',
    ...
    'Haber-Bosch',
    'other sectors',
    'animal nitrogen import-export',
    'cereals (excluding rice) food nitrogen import-export',
    'fruits and vegetables food nitrogen import-export',
    'leguminous food nitrogen import-export',
    'oleaginous food nitrogen import-export',
    'roots food nitrogen import-export',
    'rice food nitrogen import-export',
    'cereals (excluding rice) feed nitrogen import-export',
    'forages feed nitrogen import-export',
    'leguminous feed nitrogen import-export',
    'oleaginous feed nitrogen import-export',
    'grasslands feed nitrogen import-export']
...
node_color = {
    label_to_index[label]: (
        'yellow' if label in categories_mapping and categories_mapping[label] in ["cereals (excluding rice)", 'fruits and vegetables', "roots", "forages", "oleaginous"] else
        'lightgreen' if label in categories_mapping and categories_mapping[label] == "leguminous" else
        'darkgreen' if label in categories_mapping and categories_mapping[label] == "grasslands" else
        'lightblue' if label in betail else
        'darkblue' if label in ["urban", 'rural'] else
        'red' if label in ["atmospheric volatilization", "hydro-system", "other losses"] else
        'cyan' if label in ["atmospheric deposition", "N2 fixation"] else
        'purple' if label in ["Haber-Bosch", "other sectors"] else
        'gray'  # Default color if the label is not in any of the above categories
    )
    for label in labels
}
